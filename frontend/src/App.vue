        ...
        <tbody>
          <tr v-for="host in hosts" :key="host.id"
            ...
            <td>
              ...
              <p v-else @click="copyClipboard(host.host)" class="w-fit cursor-pointer self-center m-auto">{{ host.host }}
              </p>
            </td>
            <td>
              <input v-if="host.editMode" v-model="host.ip" class="w-full bg-neutral-900 text-center" />
              <p v-else @click="copyClipboard(host.ip)" class="w-fit cursor-pointer self-center m-auto">{{ host.ip }}</p>
            </td>
            <td>
              <input v-if="host.editMode" v-model="host.comment" class="w-full bg-neutral-900 text-center" />
              <p v-else class="w-fit self-center m-auto">{{ host.comment }}</p>
            </td>
            <td class="flex justify-center gap-x-2 select-none">
              <button @click="toggleEditMode(host)">{{ host.editMode ? 'Save' : 'Edit' }}</button>
